...
int main(void)
{
    ...
    while(1)
    {
        ...
        if(num1>0 && num2>0 && num3==0) min=num1;
        if(min<=ac_prev1 || min<=ac_prev2 || min<=ac_prev3 || anticheat[num1]==1 || (anticheat[num2]==1 && num2>0)|| (anticheat[num3]==1 && num3>0)|| min>checkcontinue+1 || (num3-num2>1 && num3>0) || (num2-num1>1 && num2>0))
        {
            if(min<=ac_prev1) printf("Code 1\n");
            if(min<=ac_prev2) printf("Code 2\n");
            if(min<=ac_prev3) printf("Code 3\n");
            if(anticheat[num1]==1) printf("Code 4\n");
            if(anticheat[num2]==1 && num2>0) printf("Code 5\n");
            if(anticheat[num3]==1 && num3>0) printf("Code 6\n");
            if(min>checkcontinue+1) printf("Code 7\n");
            if(num3-num2>1 && num3>0) printf("Code 8\n");
            if(num2-num1>1 && num2>0) printf("Code 9\n");
            printf("Cheating Detected, game terminated.\n");
            return 0;
        }
        if(max==nearestnum)
        {
